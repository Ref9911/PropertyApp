...
//数据库
    ...
    class Auth{
        ...
        public function getGroups($uid) {
            ...
            $user_groups =db($this->_config['AUTH_GROUP_ACCESS'])->alias('a')
                ->where($where)
                ->join(config('prefix').$this->_config['AUTH_GROUP']." g "," a.group_id=g.id")
        ...
        public function getRules($rules,$order=[]){
            ...
            $user_groups = db($this->_config['AUTH_RULE'])->where($map)->order($order)->select();
        ...
        public function getId($name){
            $authRuleId = db($this->_config['AUTH_RULE'])->where('name',$name)->field('parent_id')->find();
            if(empty($authRuleId)){
                return 0;
            }else{
        ...
        public function getFirstId($id){
            $authRuleFirstId = db($this->_config['AUTH_RULE'])->where('id',$id)->field('parent_id')->find();
        ...
        protected function getUserInfo($uid) {
            ...
            if(!isset($userInfo[$uid])){
                $userInfo[$uid]=db($this->_config['AUTH_USER'])->where('id',$uid)->find();
            }
            return $userInfo[$uid];
        }
